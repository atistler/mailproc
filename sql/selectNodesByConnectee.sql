...
<#if attributeIds?? && (attributeIds?size > 0)>
    <#list attributeIds as attributeId>
        AND nam${attributeId_index}.node_id = c.connector_node_id
    </#list>
</#if>
...
<#elseif connectorTypes?? && (connectorTypes?size > 1)>
    AND c.connector_node_type_id IN (
    <#list connectorTypes as connectorType>
        <#if (connectorType_index > 0)> , </#if> :connectorTypes[${connectorType_index}]
    </#list>
    )
</#if>
